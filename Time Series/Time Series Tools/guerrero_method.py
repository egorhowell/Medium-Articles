...
fig = px.line(
    x=lambda_values, y=likelihood_values, labels={"x": "Lambda", "y": "Log Likelihood"}
)
fig.update_layout(
    template="simple_white",
    font=dict(size=18),
    ...
    title_text="Box-Cox Likelihood Function",
    width=650,
    title_x=0.5,
    height=400,
)
...
plotting(
    title="Airline Passengers",
    data=data,
    save_file_path="passenger_box_cox.png",
    x="Month",
...
plotting(
    title="Airline Passengers",
    data=data,
    save_file_path="passenger_guerrero.png",
    x="Month",
    y="Passengers_guerrero",
    x_label="Date",
    y_label="Passengers<br>Guerrero Method",
    text=True,
    lam=transformer.lambda_,
)
