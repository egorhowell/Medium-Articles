...
def plot_time_series(df: pd.DataFrame, save_file_path: str) -> None:
...
def plot_cross_val(
    n_splits: int, splitter_func, df: pd.DataFrame, title_text: str, save_path: str
) -> None:
    ...
    plot_df = pd.DataFrame(plot_data, columns=["Index", "Dataset", "Split"]).explode(
        "Index"
    )
...
def plot_error_cv(df: pd.DataFrame, title: str, save_path: str) -> None:
