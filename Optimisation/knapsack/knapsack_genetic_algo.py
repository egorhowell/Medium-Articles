...
class GeneticAlgorithm:
    ...
    def is_valid(self, chromosome: List[int]) -> bool:
        total_weight = sum(
            self.weights[item] * chromosome[item] for item in range(len(chromosome))
        )
    ...
    def fitness(self, chromosome: List[int]) -> int:
        if not self.is_valid(chromosome):
            return 0
        return sum(
            self.values[item] * chromosome[item] for item in range(len(chromosome))
        )

    def generate_initial_population(self) -> List[List[int]]:
        population = [
            [random.randint(0, 1) for _ in range(len(self.weights))]
    ...
    def selection(self, population: List[List[int]]):
        ...
        probabilities = [
            fitness / total_fitness if total_fitness != 0 else 0
    ...
    def run(self) -> Tuple[List[int], int, List[int], int]:
        ...
        for generation in range(self.generations):
            ...
            population = sorted(
                population + children, key=lambda chromosome: -self.fitness(chromosome)
        ...
        total_weight_final = sum(
            self.weights[item] * final_solution[item]
...
hyper_parameters = HyperParameters(
    generations=2500, mutation_rate=0.1, population_size=500
)
