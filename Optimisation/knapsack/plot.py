...
fig.add_trace(
    go.Scatter(
        x=num_items, y=fortran_time, name="Fortran", line=dict(color="#3f7f93", width=4)
    )
)
fig.add_trace(
    go.Scatter(
        x=num_items, y=python_time, name="Python", line=dict(color="#fda085", width=4)
    )
)
fig.update_layout(
    title=dict(text="Execution Time Comparison", font=dict(size=24)),
    title_x=0.5,
