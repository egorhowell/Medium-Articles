...
from tabu_search_module import TabuSearch
...
# Plot the cities
fig = px.line(
    df, x="x", y="y", text="point", title="Simulated Cities: Initial Solution"
)
fig.update_layout(
    template="simple_white", font=dict(size=18), width=800, title_x=0.5, height=500
)
...
# Run our search
searcher = TabuSearch(initial_tour, cities)
best_solution = searcher.run_tabu_search(iterations=500, tabu_size=10)
...
fig = px.line(
    best_df, x="x", y="y", text="point", title="Simulated Cities: Best Found Solution"
)
fig.update_layout(
    template="simple_white", font=dict(size=18), width=800, title_x=0.5, height=500
)
