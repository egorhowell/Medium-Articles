...
# X trace
fig.add_trace(go.Scatter(x=x_X, y=kde_X(x_X), mode="lines", name="X"), row=1, col=1)

# Log(X) trace
fig.add_trace(
    go.Scatter(x=x_logX, y=kde_logX(x_logX), mode="lines", name="ln(X)"), row=1, col=2
)

# Update layout
fig.update_layout(
    title_text="Comparison of X and ln(X) Distributions",
